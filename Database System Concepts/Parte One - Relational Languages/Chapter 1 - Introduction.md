...
## 1.5 Database Design
Os sistemas de banco de dados são projetados para gerenciar grandes volumes de informações. Esses grandes volumes de informações não existem isoladamente. Eles fazem parte da operação de alguma empresa cujo produto final pode ser informação proveniente do banco de dados ou pode ser algum dispositivo ou serviço para o qual o banco de dados desempenha apenas um papel de suporte.

O design de banco de dados envolve principalmente o projeto do esquema do banco de dados. O design de um ambiente completo de aplicação de banco de dados que atenda às necessidades da empresa sendo modelada requer atenção a um conjunto mais amplo de questões. Neste texto, focamos na escrita de consultas de banco de dados e no design de esquemas de banco de dados, mas discutimos o design de aplicações posteriormente.

Um modelo de dados de alto nível fornece ao designer de banco de dados um quadro conceitual para especificar os requisitos de dados dos usuário do banco de dados e como o banco de dados será estruturado para atender esses requisitos. A fase inicial do design de banco de dados, então, é caracterizar completamente as necessidades de dados dos futuros usuários do banco de dados. O designer de banco de dados precisa interagir extensivamente com especialistas do domínio e usuários para realizar essa tarefa. O resultado dessa fase é uma especificação dos requisitos dos usuários. 

Em seguida, o designer escolhe um modelo de dados e, aplicando os conceitos do modelo de dados escolhido, traduz esses requisitos em um esquema conceitual do banco de dados. O esquema desenvolvido nessa fase de design conceitual fornece uma visão detalhada da empresa. O designer revisa o esquema para confirmar que todos os requisitos de dados estão, de fato, satisfeitos e que não estão em conflito uns com os outros. O designer também pode examinar o design para remover quaisquer características redundantes. O foco neste ponto está em <span style="background:#d4b106">descrever os dados e suas relações</span>, em vez de especificar detalhes de armazenamento físico.

Em termos de modelo relacional, o <span style="background:#b1ffff">processo de design conceitual</span> envolve <span style="background:#b1ffff">decisões sobre quais atributos queremos capturar no banco de dados</span> e como agrupar esses atributos para formas as várias tabelas. A parte *o que* é basicamente uma decisão de negócios, e não a discutiremos. A parte *como* é principalmente um problema de ciência da computação. Existem, principalmente, duas maneiras de abordar o problema.
O primeiro é usar o *modelo entidade-relacionamento* (Capítulo 6). A segunda é empregar um conjunto de algoritmos (coletivamente conhecidos como #normalização) que recebe como entrada o conjunto de todos os atributos e gera um conjunto de tabelas (Capítulo 7).

Um esquema conceitual totalmente desenvolvido indica os requisitos funcionais da empresa. Em uma especificação de requisitos funcionais, os usuários descrevem os tipos de operações (ou transações) que serão realizadas nos dados. Exemplos de operações incluem modificar ou atualizar dados, buscar e recuperar dados específicos e excluir dados. Nesta fase do design conceitual, o designer pode revisar o esquema para garantir que ele atenda aos requisitos funcionais.

O processo de passar de um modelo de dados abstrato para a implementação concreta do banco de dados prossegue em duas fases finais de design.
Na fase de *design lógico*, o designer mapeia o esquema conceitual de alto nível para o modelo de dados de implementação do sistema de banco de dados que será utilizado. O designer usa o esquema de banco de dados específico do sistema resultante na subsequente fase de *design físico*, na qual são especificados os recursos físicos do banco de dados. Esses recursos incluem a forma de organização de arquivos e as estruturas de armazenamento interno; eles são discutidos no Capítulo 13.

## 1.6 Database Engine
Um sistema de banco de dados é dividido em módulos que lidam com cada uma das responsabilidades do sistema geral. Os componentes funcionais de um sistema de banco de dados podem ser amplamente divididos em **gerenciador de armazenamento**, **componentes do processador de consultas** e o **componente de gerenciamento de transações**.

O *gerenciador de armazenamento* é importante porque os bancos de dados normalmente exigem uma grande quantidade de espaço de armazenamento. Bancos de dados corporativos geralmente variam em tamanho de centenas de gigabytes até terabytes de dados. 

Como a memória principal dos computadores não pode armazenar tanta informação, e como o conteúdo da memória principal é perdido em caso de falha do sistema, as informações são armazenadas em discos. Os <span style="background:#d3f8b6">dados são movidos entre o armazenamento em disco e a memória principal conforme necessário</span>. Como o movimento de dados para e do disco é lento em relação à velocidade da unidade central de processamento (CPU), <span style="background:#affad1">é imperativo que o sistema de banco de dados estruture os dados de forma a minimizar a necessidade de mover dados entre o disco e a memória principal</span>. Cada vez mais, **discos de estado sólido SSDs** estão sendo usados para armazenamento de banco de dados. SSDs são mais rápidos que discos tradicionais.

O *processador de consultas* é importante porque ajuda o sistema de banco de dados a simplificar e facilitar o acesso aos dados. O processador de consultas permite que os usuários do banco de dados obtenham bom desempenho enquanto trabalham no nível de visão, sem serem sobrecarregados com a compreensão dos detalhes de implementação no nível físico do sistema. É função do sistema de banco de dados traduzir atualizações e consultas escritas em uma linguagem não #procedural, no nível lógico, em uma sequência eficiente de operações no nível físico.

O SQL é considerado uma linguagem **não procedural**. Isso significa que, ao usar SQL, especificamos **o que desejamos como resultado**, mas não precisamos descrever os passos exatos de como o banco de dados deve executar a tarefa. O sistema de banco de dados é responsável por determinar a melhor forma de realizar essas operações.

O *gerenciador de transações* é importante porque permite que os desenvolvedores de aplicativos tratem uma sequência de acessos a banco de dados como se fossem uma única unidade que ocorre integralmente ou não ocorre de forma alguma. Isso permite que os desenvolvedores de aplicativos pensem em um nível mais alto de abstração sobre a aplicação, sem precisar se preocupar com os detalhes de baixo nível relacionados ao gerenciamento dos efeitos do acesso concorrente aos dados e das falhas do sistema.

Embora o mecanismo de banco de dados tradicionalmente fossem sistema de computação centralizados, hoje o **processamento paralelo** é fundamental para lidar com grandes volumes de dados de forma eficiente. <span style="background:#d4b106">Os modernos mecanismos de banco de dados</span> dedicam muita atenção ao **armazenamento de dados paralelo** e ao **processamento paralelo de consultas**.

### 1.6.1 Storage Manager
O **gerenciamento de armazenamento** é o componente de um sistema de banco de dados que fornece a interface entre os dados de baixo nível armazenados no banco de dados e os programas de aplicação e consultas enviados ao sistema. O gerenciador de armazenamento é responsável pela <span style="background:#d4b106">interação com o gerenciador de arquivos</span>. Os dados brutos são armazenados no disco utilizando o sistema de arquivos fornecido pelo sistema operacional. <span style="background:#b1ffff">O gerenciador de armazenamento traduz as várias instruções DML em comandos de baixo nível do sistema de arquivos</span>. Assim, o gerenciador de armazenamento é responsável por **armazenar**, **recuperar** e **atualizar** *os dados no banco de dados*.

Os componentes do gerenciador de armazenamento incluem:
- **Gerenciador de autorização e integridade**, que verifica o cumprimento das restrições de integridade e verifica a autoridade dos usuários para acessar os dados.
- **Gerenciador de transações**, que garante que o banco de dados permaneça em um estado consistente (correto), mesmo diante de falhas do sistema, e que as execuções de transações concorrentes prossigam sem conflitos.
- **Gerenciador de arquivos**, que gerencia a alocação de espaço no armazenamento em disco e as estruturas de dados usadas para representar as informações armazenadas no disco.
- **Gerenciador de buffer**, que é responsável por buscar dados do armazenamento em disco para a memória principal e decidir quais dados serão armazenados em cache na memória principal. O gerenciador de buffer é uma parte crítica do sistema de banco de dados, pois permite que o banco de dados manipule tamanhos de dados muito maiores do que a capacidade de memória principal. 

O gerenciador de armazenamento implementa várias estruturas de dados como parte da implementação física do sistema:
- **Arquivos de dados**, que armazenam o próprio banco de dados;
- **Dicionário de dados**, que armazena metadados sobre a estrutura do banco de dados, em particular o esquema do banco de dados;
- **Índices**, que podem fornecer acesso rápido a itens de dados. Assim como o índice neste livro-texto, um índice de banco de dados fornece ponteiros para os itens de dados que possuem um valor específico. Por exemplo, poderíamos usar um índice para encontrar o registro de um instrutor com um ID específico ou todos os registros de instrutores com um nome particular.

### 1.6.3 Transaction Management
Frequentemente, várias operações no banco de dados forma uma única unidade lógica de trabalho. Um exemplo é uma transferência de fundos, em que uma conta A é debitada e outra conta B é creditada. Claramente, é essencial que tanto o <span style="background:#d4b106">crédito quanto o débito ocorram, ou que nenhum deles ocorra</span> ( #Atomicidade ). Ou seja, a transferência de fundos deve ocorrer por completo ou não ocorrer de forma alguma. Esse requisito de *tudo ou nada* é chamado de atomicidade. Além disso, é fundamental que a execução da transferência de fundos preserve a consistência do banco de dados. Ou seja, o valor da soma dos saldos das contas A e B deve ser preservado. Esse requisito de correção é chamado de *consistência*. Finalmente, após a execução bem-sucedida de uma transferência de fundos, os novos valores dos saldos das contas A e B <span style="background:#b1ffff">devem persistir</span>, mesmo diante da possibilidade de falha do sistema. Esse requisito de persistência é chamado de #Durabilidade 

Uma *transação* é uma coleção de operações que realiza uma única função lógica em uma aplicação de banco de dados. Cada transação é uma unidade tanto de *atomicidade* quanto de *consistência*. Assim, exigimos que as transações não violem quaisquer restrições de consistência do banco de dados. Ou seja, se o banco de dados estava consistente quando uma transação começou, ele deve permanecer consistente quando a transação terminar com sucesso. No entanto, durante a execução de uma transação, pode ser necessário permitir temporariamente uma inconsistência , já que o débito de A ou o crédito de B deve ser feito antes do outro. Essa inconsistência temporária, embora necessária, pode causar dificuldades se ocorrer uma falha.

É responsabilidade do programador definir corretamente a várias transações para que cada uma preserve a consistência do banco de dados. Por exemplo, a transação para transferir fundos da conta A para a conta B poderia ser definida como composta por dois programas separados: um que debita a conta  A e outro que credita a conta B. A execução desses dois programas um após o outro, de fato, preservará a consistência. No entanto, cada programa por si só não transforma o banco de dados de um estado consistente para um novo estado consistente. Portanto, esses programas não são transações. 

Garantir as propriedades de *atomicidade* e *durabilidade* é responsabilidade do próprio sistema de banco de dados - especificamente, do **gerenciador de recuperação**. Na ausência de falhas, todas as transações são concluídas com sucesso, e a atomicidade é alcançada com sucesso. No entanto, devido a vários tipos de falhas, uma transação pode nem sempre concluir sua execução com sucesso. Se quisermos garantir a propriedade de atomicidade, uma transação falha não deve ter nenhum efeito sobre o estado do banco de dados. Assim, o banco de dados deve ser restaurado ao estado em que estava antes de a transação em questão começar a ser executada. Portanto, o sistema de banco de dados deve realizar a **recuperação de falhas**, ou seja, deve detectar falhas no sistema e restaurar o banco de dados ao estado que existia antes da ocorrência da falha.  (COMO A ANOTAÇÃO TRANSACTION ENTRA E INTERAGE NO BANCO DE DADOS)?

Finalmente, quando várias transações atualizam o banco de dados de forma concorrente, a consistência dos dados pode deixar de ser preservada, mesmo que cada transação individual seja correta. É responsabilidade do <span style="background:#b1ffff">gerenciador de controle de concorrência</span> controlar a interação entre as transações concorrentes para garantir a consistência do banco de dados. O **gerenciador de transações** consiste no gerenciador de controle de concorrência e no gerenciador de recuperação.

O conceito de transação tem sido amplamente aplicado em sistemas de banco de dados e aplicações. Embora o uso inicial de transações tenha sido em aplicações financeiras, o conceito agora é usado em aplicações em tempo real em telecomunicações, bem como no gerenciamento de atividades de longa duração, como design de produtos ou fluxos de trabalho administrativos.

## 1.7 Database and Application Architecture
Agora estamos em posição de fornecer uma visão única dos vários componentes de um sistema de banco de dados e das conexões entre eles. A figura abaixo mostra a arquitetura de um sistema de banco de dados que é executado em uma máquina servidor centralizada. A figura resume como diferentes tipos de usuários interagem com o banco de dados e como os diferentes componentes do mecanismo de banco de dados estão conectados entre si. 

![[Chapter 1 - Introduction-2.png]]

A arquitetura centralizada mostrada na figura 1.3 é aplicável a arquiteturas de servidores de memória compartilhada, que possuem múltiplos CPUs e exploram o processamento paralelo, mas todos. 

O capítulo 23 apresenta uma série de questões que surgem no processamento de transações em um banco de dados paralelo ou distribuído e descreve como lidar com cada questão. As questões incluem como armazenar dados, como garantir a atomicidade de transações que são executadas em vários sites, como realizar o controle de concorrência e como fornecer alta disponibilidade na presença de falhas. 

Agora consideramos a arquitetura de aplicações que utilizam banco de dados como backend. As aplicações de banco de dados podem ser divididas em duas ou três partes, conforme mostrado na figura 1.4. 

As aplicações de banco de dados de gerações anteriores usavam uma arquitetura de dois níveis #two-tier, onde a <span style="background:#b1ffff">aplicação residia na máquina cliente</span> e invocava a funcionalidade do sistema de banco de dados na máquina servidor por meio de declarações em linguagem de consulta. 

Em contraste, as aplicações modernas de banco de dados <span style="background:#b1ffff">utilizam uma arquitetura de três níveis</span> ( #three-tier), onde <span style="background:#b1ffff">a máquina cliente atua apenas como uma interface frontal</span> e não contém chamadas diretas ao banco de dados; os navegadores da web e os aplicativos móveis são os tipos mais comuns de cliente de aplicação hoje em dia. A interface frontal se comunica com um servidor de aplicação. O servidor de aplicação, por sua vez, se comunica com um sistema de banco de dados para acessar os dados. A lógica de negócios da aplicação, que define quais ações devem ser executadas sob quais condições, está incorporada no servidor de aplicação, em vez de ser distribuída entre múltiplos clientes. As aplicações de três níveis oferecem melhor segurança e melhor desempenho do que as aplicações de dois níveis. 

![[Chapter 1 - Introduction-3.png]]


## 1.8 Database Users and Administrators
O objetivo primário de um sistema de banco de dados é recuperar informações do banco de dados e armazenar novas informações nele. As pessoas que trabalham com um banco de dados podem ser categorizadas como *usuários de banco de dados* ou *administradores de banco de dados*.

### 1.8.1 Database Users and User Interfaces
Existem quatro tipos diferentes de usuários de sistemas de banco de dados, diferenciados pela forma como eles esperam interagir com o sistema. Diferentes tipos de interfaces de usuário foram projetadas para os diferentes tipos de usuários.
- *Usuários ingênuos:* são usuários não sofisticados que interagem com o sistema utilizando interfaces pré-definidas, como aplicativos web ou móveis. A interface de usuário típica para usuários ingênuos é uma *interface de formulários*, onde o usuário pode preencher os campos apropriados do formulário. Usuários ingênuos também podem visualizar relatórios gerados a partir do banco de dados. 
- *Programadores de aplicativos:* são profissionais de computação que escrevem programas de aplicação. Os programadores de aplicativos podem escolher entre várias ferramentas para desenvolver interfaces de usuário.
- *Usuários sofisticados* interagem com o sistema sem escrever programas. Em vez disso, eles formam suas requisições utilizando uma **linguagem de consulta de banco de dados** ou utilizando ferramentas como software de análise de dados. Analistas que enviam consultas para explorar dados no banco de dados se enquadram nessa categoria. 

### 1.8.2 Database Administrator
Um dos principais motivos para o uso de SGBDs (Sistemas de Gerenciamento de Banco de Dados) é ter controle centralizado tanto dos dados quanto dos programas que acessam esses dados. A pessoa que possui esse controle central sobre o sistema é chamada de **administrador de banco de dados DBA**. As funções de um DBA incluem:
- **Definição do esquema:** o DBA cria o esquema original do banco de dados executando um conjunto de instruções de definição de dados DDL (Linguagem de Definição de Dados);
- **Definição das estruturas de armazenamento e métodos de acesso:** o DBA pode especificar alguns parâmetros relacionados à organização física dos dados e aos índices a serem criados.
- **Concessão de autorização para acesso aos dados**: ao conceder diferentes tipos de autorização, o administrador de banco de dados pode regular quais partes do banco de dados os diversos usuários podem acessar. As informações de autorização são mantidas em uma estrutura de sistema especial que o sistema de banco de dados consulta sempre que um usuário tenta acessar os dados no sistema.
- **Manutenção rotineira**: exemplos das atividades de manutenção rotineira do administrador de banco de dados incluem:
- Fazer backups periódicos do banco de dados em servidores remotos, para evitar a perda de dados em caso de desastres, como inundações;
- Garantir que haja espaço suficiente em disco disponível para operações normais e atualizar o espaço em disco conforme o necessário.
- Monitorar os trabalhos em execução no banco de dados e garantir que o desempenho esteja dentro dos padrões aceitáveis.